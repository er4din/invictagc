...
- **Code changes** (themes, plugins) — pushed via git to GitHub, automatically deployed to the live server via GitHub Actions
...
3. Enable WordPress maintenance mode on the live site — upload your `.maintenance` file to the `public_html/` root via GreenGeeks cPanel File Manager. This activates the branded maintenance page (`wp-content/maintenance.php`) for all visitors while allowing the database swap to proceed. Every developer keeps a personal copy of this file on hand. Do not commit `.maintenance` to git — it is gitignored to prevent it from being accidentally deployed and locking users out permanently
...
The Replace value for localhost must always use `http://` — never `https://`. Localhost has no SSL certificate, so entering `https://localhost/invictagc` will cause an SSL error on every page and make the local site inaccessible.
